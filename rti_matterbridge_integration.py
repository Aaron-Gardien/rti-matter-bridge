...
class RTIMatterbridgeIntegration:
    def __init__(self, rti_host: str = "192.168.102.232", rti_port: int = 1234,
                 ...
                 switch_config_path: Optional[str] = None):
        ...
        self.switch_plugin = RtiSwitchPlugin.from_file(switch_config_path) if switch_config_path else None
    ...
    async def subscribe_tracked_variables(self):
        ...
        variable_ids = set(self.tracked_variables)
        if self.switch_plugin:
            variable_ids.update(self.switch_plugin.tracked_variable_ids())
    ...
    async def handle_rti_message(self, message: str):
        ...
        try:
            ...
            if data.get("messageType") == "Sysvar":
                ...
                if self.switch_plugin:
                    switch_update = self.switch_plugin.handle_feedback(numeric_variable_id, variable_value)
                    if switch_update:
                        await self.send_to_matterbridge("rti_switch_state_update", switch_update)
    ...
    async def handle_matter_device_command(self, data: Dict[str, Any]):
        ...
        # Map to RTI macro
        rti_macro = self.switch_plugin.macro_for_command(device_id, command) if self.switch_plugin else None
        if not rti_macro:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "switches": len(self.switch_plugin.switches) if self.switch_plugin else 0,
...
def parse_args():
    ...
    parser.add_argument("--switch-config", help="Optional RTI switch config generated by rti_switch_config_generator.py.")
...
async def main():
    """Main function"""
    args = parse_args()
    integration = RTIMatterbridgeIntegration(mapping_path=args.mapping, switch_config_path=args.switch_config)
